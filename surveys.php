...
if ($role === 'admin' && isset($_POST['add_survey'])) {
    ...
    mysqli_query($con, "INSERT INTO surveys (title, link) VALUES ('$title','$link')");
}
...
<?php
$surveys = mysqli_query($con, "SELECT * FROM surveys ORDER BY id DESC");
...
while ($survey = mysqli_fetch_assoc($surveys)):
    ...
    // Total users
    $total_users_query = mysqli_query($con, "SELECT COUNT(*) as total FROM users");
    ...
    <div class="mb-4">
        ...
        <?php if ($role === 'user'): ?>
            <?php if (mysqli_num_rows($user_check) > 0): ?>
                <span class="ml-3 bg-green-500 text-white px-4 py-2 rounded-lg">
                    Completed ✓
                </span>
            <?php else: ?>
                <a href="?complete=<?php echo $survey_id; ?>"
                   class="ml-3 bg-red-500 text-white px-4 py-2 rounded-lg hover:bg-red-600">
                   Mark Completed
                </a>
            <?php endif; ?>
        <?php endif; ?>
